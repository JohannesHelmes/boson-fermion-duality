...
for equalsum in range(2*order_min,MaxSUM+1):
    ...
    for i,(Lx,Ly) in enumerate(order.clusters_fix_sum(order_max,equalsum)):
        ...
        if weights[Lx,Ly].all()==0:
            ...
            #assert Ly>=Lx
            i=0
            #Could be done more efficiently in this particular case ...
            for y0 in range(1,Ly):
                for x0 in range(1,Lx):
                    if rank==i%size:
                        local_diaEntros[x0,y0]= free_boson_2D.getEntropy(L,alpha,free_boson_2D.getRegionA(L,(x0,y0),2,angle),X,P)
                    i+=1
            ...
            for y0 in range(1,Ly):
                for x0 in range(1,Lx):
                    r0=(x0,y0)
                    if rank==i%size:
                        local_cornerEntros[x0,y0]= free_boson_2D.getEntropy(L,alpha,free_boson_2D.getRegionA(L,r0,3,angle),X,P)
                    i+=1
            ...
            if rank==0:
                for y0 in range(1,Ly):
                    for x0 in range(1,Lx):
                        ...
                        '''
                        print r0
                        print free_boson_2D.getRegionA(L,r0,1,angle).T
                        print free_boson_2D.getRegionA(L,r0,2,angle).T
                        print free_boson_2D.getRegionA(L,r0,3,angle).T
                        '''
                        #corners:
