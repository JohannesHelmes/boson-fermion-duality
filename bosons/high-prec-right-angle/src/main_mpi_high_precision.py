...
import time
import free_boson_2D_high_precision as free_boson_2D
...
def getXandP(Lx,Ly,massterm):
    ...
    precK=mpm.mp.matrix(K)

    SqrtK=mpm.sqrtm(precK)
    P = 1./2. * SqrtK
    X = 1./2. * (SqrtK ** -1)
...
pre_alphas=mpm.linspace(1,5,41)[1:] #exclude the 1.0 to avoid doubling, add it later
alpha=[]
for a in pre_alphas:
    alpha.append(1/a)
alpha.append(mpm.mpf('1.0'))
for a in pre_alphas:
    alpha.append(a)
#alpha.sort()
        ...
        if weights[Lx,Ly].all()==0:
            ...
            for i,y0 in enumerate(range(1,Ly/2+1)):
                if rank==i%size:
                    while True:
                        try:
                            local_horEntros[y0]= free_boson_2D.getEntropy(L,alpha,free_boson_2D.getRegionA(L,(0,y0),2,angle),X,P)
                            mpm.mp.dps=digit_precision
                            break
                        except RuntimeError:
                            mpm.mp.dps+=1
                            print "Rank", rank, "Did not converge, precision increased to ",mpm.mp.dps
                            X,P=getXandP(Lx,Ly,massterm)
            ...
            for i,x0 in enumerate(range(1,Lx)):
                if rank==(i+Ly/2+1)%size:
                    while True:
                        try:
                            local_verEntros[x0]= free_boson_2D.getEntropy(L,alpha,free_boson_2D.getRegionA(L,(x0,0),1,angle),X,P)
                            mpm.mp.dps=digit_precision
                            break
                        except RuntimeError:
                            mpm.mp.dps+=1
            ...
            for y0 in range(1,(Ly+1)/2):
                for x0 in range(1,Lx):
                    ...
                    if rank==i%size:
                        while True:
                            try:
                                local_cornerEntros_ll[x0,y0]= free_boson_2D.getEntropy(L,alpha,free_boson_2D.getRegionA(L,r0,4,angle),X,P)
                                mpm.mp.dps=digit_precision
                                break
                            except RuntimeError:
                                mpm.mp.dps+=1
                    ...
                    if rank==i%size:
                        while True:
                            try:
                                local_cornerEntros_ur[x0,y0]= free_boson_2D.getEntropy(L,alpha,free_boson_2D.getRegionA(L,r0,3,angle),X,P)
                                mpm.mp.dps=digit_precision
                                break
                            except RuntimeError:
                                mpm.mp.dps+=1
            ...
            if rank==0:
                ...
                for i in range(len(alpha)):
                  fout_w.write("%.24e\t" %weights[Lx,Ly][i]) #PRECISION IMPORTANT IN OUTPUT
                fout_w.write(" ]\n")
                fout_w.flush()
...
if rank==0:
    ...
    for ord in range(order_min,order_max+1):
        ...
        # Save result to file
        for i in range(len(alpha)):
            fout_res[i].write("%d %.32f"%(ord,total[i])+'\n')
            fout_res[i].flush()
