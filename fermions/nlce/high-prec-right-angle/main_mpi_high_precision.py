import clust_order
import free_fermion_2D_high_precision as free_fermion_2D
import numpy as np
...
def generateHamiltonian(Lx,Ly,massterm):
    ...
    upper_tri=(mpm.matrix(diag)+mpm.matrix(offdiag_x)+mpm.matrix(offdiag_y))

    lower_tri=mpm.matrix(upper_tri.cols,upper_tri.rows)

    for y in range(upper_tri.rows):
        for x in range(upper_tri.cols):
            lower_tri[x,y]=mpm.conj(upper_tri[x,y])

    lower_tri=lower_tri.T

    Ham=upper_tri+lower_tri
...
massterm = mpm.mpf('1.0')
#############################


clusters = []
alpha=1./np.array( np.linspace(0.5,4,36).tolist() )
    ...
    for i,(Lx,Ly) in enumerate(order.clusters_fix_sum(order_max,equalsum)):
        ...
        if weights[Lx,Ly].all()==0:
            ...
            for i,y0 in enumerate(range(1,Ly/2+1)):
                if rank==i%size:
                    local_horEntros[y0]= free_fermion_2D.getEntropy(L,alpha,free_fermion_2D.getRegionA(L,(0,y0),2),Hamiltonian)
            
            for i,x0 in enumerate(range(1,Lx)):
                if rank==(i+Ly/2+1)%size:
                    local_verEntros[x0]= free_fermion_2D.getEntropy(L,alpha,free_fermion_2D.getRegionA(L,(x0,0),1),Hamiltonian)
            ...
            for y0 in range(1,(Ly+1)/2):
                for x0 in range(1,Lx):
                    r0=(x0,y0)
                    if rank==i%size:
                        local_cornerEntros_ll[x0,y0]= free_fermion_2D.getEntropy(L,alpha,free_fermion_2D.getRegionA(L,r0,4),Hamiltonian)
                    i+=1
                    if rank==i%size:
                        local_cornerEntros_ur[x0,y0]= free_fermion_2D.getEntropy(L,alpha,free_fermion_2D.getRegionA(L,r0,3),Hamiltonian)
                    i+=1
